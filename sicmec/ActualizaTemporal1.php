...
//SELECCIONA TODO EL EQUIPO QUE HAY EN LA TABLA DE TEMPORAL
    ...
    while ($row_constemp = mysql_fetch_assoc($constemp)) {
        $query_cons = sprintf("SELECT * FROM actfijo WHERE CveCatalogo = '%s' AND NumInv = %s AND AnnoAlta= %s AND NumSerie = '%s'", $row_constemp['CveCatalogo'], $row_constemp['NumInv'], $row_constemp['AnnoAlta'], trim($row_constemp['NumSerie']));
        ...
        if ($totalRows_cons > 0) {
            $updateSQL = sprintf("UPDATE actfijo SET CaractAcf='%s', CveAdscrip='%s', NumSerie='%s' WHERE NumInv = %s And CveCatalogo = '%s' AND AnnoAlta = %s", $row_constemp['CaractAcf'], trim($row_constemp['CveAdscrip']), trim($row_constemp['NumSerie']), $row_constemp['NumInv'], $row_constemp['CveCatalogo'], $row_constemp['AnnoAlta']);
            ...
            $insertSQL = sprintf("INSERT INTO actfijo (CveCatalogo, NumInv, AnnoAlta, CaractAcf, CveHist, CveStatus, CveUbic, CveAdscrip, NumSerie) VALUES ('%s', %s, %s, '%s',0, %s, 2,'%s', '%s')", $row_constemp['CveCatalogo'], $row_constemp['NumInv'], $row_constemp['AnnoAlta'], $row_constemp['CaractAcf'], $status, trim($row_constemp['CveAdscrip']), trim($row_constemp['NumSerie']));
